...
** Step 1 — Translate the config

Run this with the *old* ~config.lisp~ still in place:
...
~connection-spec~ format, writes the result to ~new-config.lisp~, and
prints a checklist.  Review ~new-config.lisp~, then copy it into place
as ~config.lisp~.
