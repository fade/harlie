...
- [X] Smart rejoin.  Detect disconnects from server.
  A: Done.  clatter-irc provides auto-reconnect with exponential backoff;
  harlie adds a keepalive PING thread.
...
- [X] Calendars, reminders within the bot
  A: Basic !remind plugin implemented.
